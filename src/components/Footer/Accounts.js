...
class Accounts extends Component {
  ...
  static propTypes = {
    items: PropTypes.shape({
      ...
      isOdnoklassnikiConnected: PropTypes.bool.isRequired,
      isVkConnected: PropTypes.bool.isRequired,
      isYandexConnected: PropTypes.bool.isRequired,
  ...
  render() {
    ...
    if (items !== null) {
      ...
      count = items.isOdnoklassnikiConnected ? count + 1 : count;
      count = items.isVkConnected ? count + 1 : count;
      count = items.isYandexConnected ? count + 1 : count;
      ...
      return (
        <ul className={cx(s.listStyle, s.space1, 'verificationBox', 'listStyleRtl')}>
          ...
          {
                        items.isYandexConnected && <AccountItem itemName={formatMessage(messages.yandexConnected)} />
                    }
          {
                        // items.isOdnoklassnikiConnected && <AccountItem itemName={formatMessage(messages.odnoklassnikiConnected)} />
                    }
          {
                        items.isVkConnected && <AccountItem itemName={formatMessage(messages.vkConnected)} />
                    }
          {
                        items.isIdVerification && <AccountItem itemName={formatMessage(messages.documentverified)} />
                    }
          {
                        !items.isEmailConfirmed && !items.isFacebookConnected && !items.isVkConnected && !items.isYandexConnected && !items.isOdnoklassnikiConnected && !items.isGoogleConnected && !items.isIdVerification && <NoItem isLoggedInUser={isLoggedInUser} />
                    }
          {
