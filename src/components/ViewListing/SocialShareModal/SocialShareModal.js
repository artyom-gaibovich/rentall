...
class SocialShareModal extends Component {
  ...
  render() {
    ...
    const locationPath = location ? location.replace('/preview', '') : null;

    const shareUrl = url + locationPath;
    const previewText = `Check out this listing on ${siteName}!`;
    const bodyText = `${`Check out this listing on ${siteName}!` + ' '}${shareUrl}`;
