...
class ThreadItems extends Component {
  ...
  render() {
    const { data, data: { threadItems, threadItemsCount }, userType, loadMore, isAdminAuthenticated } = this.props;
        ...
        <div>
          {
            threadItems != null && threadItems.length > 0 && threadItems.map((item, index) => (
              <div key={index} className={cx(s.displayFlex)}>
                <div className={cx(s.displayFlexGrow)}>
                  {
                ...
                </div>
                {
                  ...
                  (item.type === 'inquiry' || item.type === 'message') &&
                  (
                    (userType === 'host' && item.sentBy === data.guest) ||
                    (userType === 'guest' && item.sentBy === data.host)
                  ) && <CustomCheckbox
