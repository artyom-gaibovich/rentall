...
class UserManagement extends React.Component {
  ...
  async handleChange(e, userId, userMail, userName) {
    ...
    const id = userId;
    const banStatus = e.target.value;
            ...
            >
              ...
              </Thead>
              {
                userManagement && userManagement.usersData.length > 0 && userManagement.usersData.map((value, key) => {
                  ...
                  const userName = value.profile && `${value.profile.firstName} ${value.profile.lastName}`;
                  if (banStatus === 1) {
                    banStatus = '1';
                  } else if (banStatus === 0) {
                    banStatus = '0';
                    ...
                    <Tr key={key}>
                      ...
                      </Td>
                      {
                        <Td data-label={formatMessage(messages.actionLabel)} column={formatMessage(messages.actionLabel)}>
                          <select
                            name="userBanStatus" className={cx(bt.commonControlSelect, s.userVerticalAlign, s.btnMarginBottom)}
