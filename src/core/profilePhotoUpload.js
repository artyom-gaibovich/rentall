...
const profilePhotoUpload = (app) => {
  ...
  app.post('/uploadProfilePhoto', (req, res, next) => {
    if (!req.user) {
      res.send(403);
    } else {
      next();
    ...
    await new Promise((resolve, reject) => {
      sharp(file.path)
        ...
        .toFile(`${profilePhotouploadDir}small_${file.filename}`, (err, file) => {
          if (file) {
            resolve(file);
          } else {
            reject(err);
    ...
    await new Promise((resolve, reject) => {
      sharp(file.path)
        ...
        .resize(255, 255)
        // .crop(sharp.strategy.entropy)
        .toFile(`${profilePhotouploadDir}medium_${file.filename}`, (err, file) => {
          if (file) {
            resolve(file);
          } else {
            reject(error);
  ...
  app.post('/deleteProfilePicture', (req, res, next) => {
    if (!req.user) {
      res.send(403);
    } else {
      next();
