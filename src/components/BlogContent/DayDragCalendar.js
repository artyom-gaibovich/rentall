...
class DayDragCalendar extends Component {
  ...
  renderDay(day) {
    ...
    const dateRangeValue = moment(day).format('YYYY-MM-DD');
  ...
  render() {
    ...
    const listCountryDate = getDateUsingTimeZone(country, false);
    const today = new Date(listCountryDate.get('year'), listCountryDate.get('month'), listCountryDate.get('date'), listCountryDate.get('hour'), listCountryDate.get('minute'), listCountryDate.get('second'));
