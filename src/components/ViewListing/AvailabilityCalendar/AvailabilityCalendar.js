...
class AvailabilityCalendar extends React.Component {
  ...
  componentDidMount() {
    ...
    const windowHeight = isBrowser ? window.matchMedia('(max-width: 640px)').matches : undefined;
  ...
  async onNextMonthChange(e) {
    ...
    const { blockedDatesSet } = this.state;
    const today = getDateUsingTimeZone(country, true);
    const navigationDate = moment(e).format('YYYY-MM-DD');
    const monthsDiff = moment(navigationDate).diff(moment(today), 'months');
    // const blockedDatesSet = new Set();
    if (monthsDiff && monthsDiff > 0) {
      const isRangedMonth = Number(monthsDiff) % 5;
      if (isRangedMonth === 0) {
  ...
  async onPrevMonthChange(e) {
    ...
    const monthsDiff = moment(navigationDate).diff(moment(today), 'months');
    // const blockedDatesSet = new Set();
    const filterDate = moment(e).add(-5, 'months');

    if (monthsDiff && monthsDiff > 0) {
      const isRangedMonth = Number(monthsDiff) % 4;
      if (isRangedMonth === 0) {
  ...
  handleResize(e) {
    const isBrowser = typeof window !== 'undefined';
    const smallDevice = isBrowser ? window.matchMedia('(max-width: 767px)').matches : false;
  ...
  render() {
    ...
    let today = getDateUsingTimeZone(country, false),
      condition;
    const breakPoint = getDateUsingTimeZone(country, false);
    const minNight = listingData && listingData.minNight;
    const maxNight = listingData && listingData.maxNight;
    const maxDaysNotice = listingData && listingData.maxDaysNotice;
    const bookingNoticeTime = listingData && listingData.bookingNoticeTime;
