...
class HostDetail extends React.Component {
  ...
  render() {
    const { contactHostOpen, isHost, hostEmail, userBanStatus, urlParameters, isAuthenticated } = this.props;
            ...
            <div className={s.displayTable}>
              <div className={s.displayTableRow}>
                <div className={cx(s.displayTableCell, s.vrAlignTop)}>
                  ...
                  {
                    false && !isHost && !userBanStatus && <Button className={cx(s.btn, bt.btnLarge, bt.btnPrimary)} onClick={() => isAuthenticated ? contactHostOpen(id, urlParameters) : processAuthAction() }>
                      asd123312<FormattedMessage {...messages.contactHost} />
                    </Button>
                  }
                </div>
