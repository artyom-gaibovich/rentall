...
import yandexAuth from './core/auth/yandex';
import vkAuth from './core/auth/vk';
import odnoklassnikiAuth from './core/auth/odnoklassniki';
...
import yookassaRoutes from './core/payment/yookassa';
...
import yookassaAuth from './core/auth/yookassa';
...
app.get('*', async (req, res, next) => {
  try {
    // console.log('register income request *', req.body)
    ...
    if (req.user != null && req.user != undefined && req.user.admin != true) {
      ...
      store.dispatch(setRuntimeVariable({
        name: 'yookassaConnect',
        value: req.cookies.yookassaConnect || false,
      }));
    ...
    const currentLocation = req.path;
    const collectionArray = ['/message/', '/users/trips/itinerary/', '/review/write/'];
