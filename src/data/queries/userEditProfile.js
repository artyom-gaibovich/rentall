...
const userEditProfile = {
  ...
  async resolve({ request, response }, {
    ...
    if (request.user && request.user.admin != true) {
      ...
      const updatedFirstName = capitalizeFirstLetter(firstName);
      const updatedLastName = capitalizeFirstLetter(lastName);
      const displayName = `${updatedFirstName} ${updatedLastName}`;
      const isPhoneNumber = phoneNumber || isUserPhoneNumber.phoneNumber;
      const isCountryCode = countryCode || isUserPhoneNumber.countryCode;
      const isCountryName = countryName || isUserPhoneNumber.countryName;
      ...
      if (loggedinEmail != email) {
        ...
        const token = jwt.sign({ id: request.user.id, email }, auth.jwt.secret, { expiresIn });
