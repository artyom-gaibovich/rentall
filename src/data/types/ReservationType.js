  ...
  fields: {
    ...
    hostData: {
      ...
      async resolve(reservation) {
        const hostData = await UserProfile.findOne({
          where: { userId: reservation.hostId },
          raw: true,
        });
        const phoneNumberEncrypt = { phoneNumber: encode(hostData.phoneNumber) };
    ...
    guestData: {
      type: ProfileEncryptType,
      async resolve(reservation) {
        const guestData = await UserProfile.findOne({
          where: { userId: reservation.guestId },
          raw: true,
        });

        const phoneNumberEncrypt = { phoneNumber: encode(guestData.phoneNumber) };
