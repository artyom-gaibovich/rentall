...
class GrossEarningItem extends Component {
  ...
  render() {
    ...
    const date = data.hostTransaction != null ? moment(data.hostTransaction.createdAt).format('DD-MM-YYYY') : 'Pending';
    const checkInDate = data.checkIn != null ? moment(data.checkIn).format('MMM DD, YYYY') : '';
    const checkOutDate = data.checkOut != null ? moment(data.checkOut).format('MMM DD, YYYY') : '';
    const totalAmount = Number(data.total) - Number(data.hostServiceFee);
		// let totalAmount = Number(data.total);

    const payoutAmount = data && data.cancellationDetails && data.cancellationDetails;
