...
const mobileRoutes = (app) => {
  ...
  app.post('/sendEmailTemplate', (req, res, next) => {
    next();
  }, async (req, res) => {
    let status = 200,
      errorMessage;
  ...
  app.post('/deleteListPhotos', async (req, res, next) => {
    next();
  }, async (req, res) => {
    let status = 200,
      errorMessage,
          ...
          if (isListingAvailable) {
            ...
            if (getReservationCount > 0) {
              const listPhotosCount = await ListPhotos.count({
                where: { listId },
                include: [
                  { model: Listing, as: 'listing', where },
                ],
              });
            ...
            if (checkForRemove) {
              ...
              if (removePhoto) {
                ...
                if (photosCount < 1) {
                  ...
                  const updateListStatus = await WishList.update({
                    isListActive: false,
                  }, {
                    where: {
                      listId,
          ...
          if (userProfilePicture) {
            ...
            const smallImage = await new Promise((resolve, reject) => {
              sharp(file.path)
                ...
                .toFile(`${profilePhotouploadDir}small_${file.filename}`, (err) => {
                  if (file) {
                    resolve(file);
                  } else {
                    reject(error);
            ...
            const mediumImage = await new Promise((resolve, reject) => {
              sharp(file.path)
                ...
                .resize(255, 255)
                // .crop(sharp.strategy.entropy)
                .toFile(`${profilePhotouploadDir}medium_${file.filename}`, (err) => {
                  if (file) {
                    resolve(file);
                  } else {
                    reject(error);
  ...
  app.post('/deleteMultiFiles', (req, res, next) => {
    next();
  }, async (req, res) => {
    let status = 200,
      errorMessage;
