  ...
  return async (dispatch, getState, { client }) => {
    ...
    try {
      ...
      if (data && data.UserListing) {
        ...
        // Preparing for fish
        if (data.UserListing.fish.length > 0) {
          data.UserListing.fish.map((item, value) => {
            fish.push(parseInt(item.fishId));
          });
          settingFieldsData = Object.assign({}, listData, { fish });
        }

        // Preparing for blocked dates
        if (data.UserListing.blockedDates.length > 0) {
          data.UserListing.blockedDates.map((item, value) => {
            const date = moment(item.blockedDates).format;
