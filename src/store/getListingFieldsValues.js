...
export function getListingFieldsValues(step, listId, isOtherPage) {
  return async (dispatch, getState, { client }) => {
    ...
    try {
      ...
      const listingFieldsValues = {};
      const bedType = getState().listingFields.data.bedType;
      let bedTypeValue;
      ...
      if (!data && !data.getListingSettings) {
        ...
        data.getListingSettings.map((item, key) => {
          ...
          } else if (item.isMultiValue === true) {
            listingFieldsValues[item.typeName] = [];
          } else {
            const unSelectIndex = item.listSettings.length > 0 ? item.listSettings.findIndex(o => (o.isEnable == 1)) : null;
        ...
        if (step === '2' && !step2DataIsLoaded) {
          const updatedValuesStep2 = Object.assign({}, listingFieldsValues, { id: listId });
          ...
          const updatedValuesStep3 = Object.assign({}, listingFieldsValues, { id: listId }, { currency: baseCurrency }, { bookingType: 'instant' }, { maxDaysNotice: 'available' });
          dispatch(initialize('ListPlaceStep3', updatedValuesStep3, true));
          dispatch({
            type: GET_LISTING_FIELDS_INTIAL_VALUES_SUCCESS,
            initialValuesLoadedStep3: true,
          });
        } else if (!step1DataIsLoaded || isOtherPage) {
          const updatedValuesStep1 = Object.assign({}, listingFieldsValues, { bedTypes: [{ bedCount: 1, bedType: bedTypeValue }] });
