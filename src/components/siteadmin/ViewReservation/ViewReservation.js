...
class ViewReservation extends React.Component {
  ...
  render() {
    ...
    const nextDay = moment(data.checkIn).add(1, 'days');
    const today = moment();
    const dayDifference = nextDay.diff(today, 'days');
    ...
    const taxRate = data && data.taxRate && data.taxRate > 0 ? data.taxRate : 0;

    const url = type === 'reservation' ? '/siteadmin/reservations' : '/siteadmin/payout';
    const titleMessageKey = type === 'reservation' ? 'reservationDetails' : 'payoutDetails';
    return (
