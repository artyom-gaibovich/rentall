...
class PhoneVerificationModal extends Component {
  ...
  formatPhoneNumber(phoneNumber) {
    ...
    if (phoneNumber && phoneNumber != '') {
      if (phoneNumber.length > 6) {
        ...
        formattedNumber += phoneNumber.substr(phoneNumber.length - 3, phoneNumber.length);
      } else {
  ...
  renderAddPhoneNumber() {
    ...
    return (
      <div>
        ...
        <a
          href="javascript:void(0)" className={cx(s.modalCaptionLink)}
  ...
  render() {
    ...
    const verificationStatus = !!((getPhoneData && getPhoneData.verification && getPhoneData.verification.isPhoneVerified == true));
