...
import {searchResultsData} from "../../../actions/getSearchResults"
import {
  withGoogleMap,
  GoogleMap,
  Marker,
...
iconCaption: "123",
  ...
  componentDidMount() {
    ...
    // script.src = "https://api-maps.yandex.ru/2.1/?lang=ru_RU&amp;apikey=1e734782-b0c1-4ebb-a1a3-031c8f233240";
    ...
    if (results && results.length > 0) {
      ...
      if (personalized && personalized.lat && personalized.lng) {
        ...
        const defaultCordinates = new google.maps.LatLng(center.lat, center.lng);
        bounds.extend(defaultCordinates);
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (results && results.length > 0) {
      ...
      results.map((item) => {
        const data = {};
        if (markerHighlight) {
          hover = markerHighlight.id == item.id;
        }
      ...
      // console.log('SEARCH res', results, positions, bounds)  
  ...
  handleBoundsChanged() {
    const center = new google.maps.getCenter();
  }
