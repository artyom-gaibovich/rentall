...
export async function users(keyword, userType) {
  ...
  if (keyword && keyword.length > 0 && keyword.toString().trim() != '') {
    const getDate = moment(keyword).format('YYYY-MM-DD');
  ...
  if (data && data.length > 0) {
    dataItems = await Promise.all(data.map((item) => {
      const consolidatedData = Object.assign(
        {
          Email: item.email,
...
export async function listings() {
  ...
  if (data && data.length > 0) {
    dataItems = data.map((item) => {
      const listingOtherData = item.listingData ? item.listingData.dataValues : {};
      const hostEmail = item.user ? item.user.dataValues.email : {};
      const consolidatedData = Object.assign(
                item.dataValues,
                { hostEmail },
                { listingData: '' },
