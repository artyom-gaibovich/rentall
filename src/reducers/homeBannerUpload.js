...
const homeBannerUpload = (app) => {
  ...
  app.post('/uploadHomeBanner', (req, res, next) => {
    if (!req.user) {
      res.send(403);
    } else {
      next();
    ...
    if (file.mimetype === 'image/jpeg') {
      sharp(file.path)
        .resize(1280, null)
        .jpeg({ quality: 50 })
        .toFile(`${homebanneruploadDir}xx_large_${file.filename}`, (err) => {
      ...
      sharp(file.path)
        .resize(48, null)
        .toFile(`${homebanneruploadDir}placeholder_${file.filename}`, (err) => {
          // console.log('Error from resizing files', err);
        });
  ...
  app.post('/deleteHomeBanner', (req, res, next) => {
    if (!req.user) {
      res.send(403);
    } else {
      next();
    ...
    const filePath = homebanneruploadDir;
