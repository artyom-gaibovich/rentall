            ...
            >
              ...
              </Thead>
              {
                getAllListings && getAllListings.usersData.length > 0 && getAllListings.usersData.map((value, key) => {
                  const viewListing = `/rooms/${value.id}`;
                  const editListing = `/become-a-host/${value.id}/home`;
                  const isPublished = value.isPublished ? 'Yes' : 'No';
                  const isReady = value.isReady ? 'Yes' : 'No';
                  return (
                    <Tr key={key}>
