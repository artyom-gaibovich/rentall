            ...
            >
              ...
              </Thead>
              {
                                data && data.map((value, key) => {
                                  const content = value.reviewContent;
                                  const reviewContent = content.slice(0, 10);
                                  const dots = '...';
                                  let isContent = false;
                                  ...
                                  const hostName = value.userData && value.userData.firstName;
                                  const guestName = value.authorData && value.authorData.firstName;
                                  const hostProfileId = value.userData && value.userData.profileId;
                                  const guestProfileId = value.authorData && value.authorData.profileId;
                                  const title = value.listData && value.listData.title ? value.listData.title : 'List is missing';
                                  const confirmationCode = value.singleReservationData && value.singleReservationData.confirmationCode ? value.singleReservationData.confirmationCode : '';
                                  const checkInDate = value.singleReservationData && value.singleReservationData.checkIn ? moment(value.singleReservationData.checkIn).format('DD-MM-YYYY') : '';
                                  const checkOutDate = value.singleReservationData && value.singleReservationData.checkOut ? moment(value.singleReservationData.checkOut).format('DD-MM-YYYY') : '';
