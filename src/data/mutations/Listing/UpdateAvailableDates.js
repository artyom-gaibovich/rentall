      ...
      if (blockedDates) {
        ...
        if (blockedDates && blockedDates.length > 0) {
          ...
          await Promise.all(blockedDates.map(async (item, key) => {
            ...
            const blockedItem = availableDatesItems.map(Number).indexOf(+day);
            if (blockedItem > -1) {
              const blockedDatesFind = await ListCalendarPrice.findAll({
                where: {
                  listId,
              ...
              const createRecord = await ListCalendarPrice.findOrCreate({
                where: {
                  listId,
                  blockedDates: item,
                  isSpecialPrice,
          ...
          await Promise.all(blockedDates.map(async (item, key) => {
            const updateAvailableDates = await ListCalendarPrice.findOrCreate({
              where: {
                listId,
                blockedDates: item,
                isSpecialPrice,
