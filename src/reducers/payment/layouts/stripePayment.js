...
const stripePayment = (app) => {
  app.post('/stripe-reservation', async (req, res) => {
    ...
    const redirect = `/users/trips/itinerary/${reservationDetails.reservationId}`;
    res.send({ status, errorMessage, redirect, paymentIntentSecret });
  });

  app.post('/stripe-reservation-confirm', async (req, res) => {
  ...
  app.get('/stripe-get-customer', async (req, res) => {
  ...
  app.post('/stripe-create-source', async (req, res) => {
        // const customer = await stripe.customers.retrieve('cus_BWAA082C5iaGXF1');
