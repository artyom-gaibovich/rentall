      ...
      if (isListingAvailable) {
        ...
        if (blockedDates && blockedDates.length > 0) {
          ...
          blockedDates.map(async (item) => {
            const oldRecords = reservationDatesCollection.find(v => moment(item).format('YYYY-MM-DD') === moment(v.blockedDates).format('YYYY-MM-DD'));
            const checkDate = reservationDatesCollection.indexOf(item);
