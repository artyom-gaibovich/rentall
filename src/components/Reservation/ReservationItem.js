...
class ReservationItem extends Component {
  ...
  render() {
    ...
    const checkInDate = checkIn ? moment(checkIn).format('Do MMM - ') : '';
    const checkOutDate = checkOut ? moment(checkOut).format('Do MMM, YYYY') : '';
    const createdDate = createdAt ? moment(createdAt).format('Do MMM, YYYY') : '';
    let subTotal = 0;
    let today = getDateUsingTimeZone(country, false),
      formattedCheckout = moment(checkOut);
    ...
    if (userType === 'host') {
      subTotal = hostServiceFee;
    } else {
      subTotal = guestServiceFee;
    }
