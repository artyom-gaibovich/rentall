...
class DetailsForHost extends React.Component {
  ...
  render() {
    ...
    const checkInDate = checkIn != null ? moment(checkIn).format('Do MMM YYYY') : '';
    const checkOutDate = checkOut != null ? moment(checkOut).format('Do MMM YYYY') : '';
    let refundAmount = 0,
    ...
    const coverImage = holdeData && holdeData.listData && holdeData.listData.listPhotos.find(o => o.id == holdeData.listData.coverPhoto);
    ...
    totalPrice = isSpecialPriceAverage || basePrice;
    const checkInNights = remainingNights || nights;
