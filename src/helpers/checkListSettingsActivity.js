import {
    ...
    UserSafetyAmenities, UserHouseRules, UserFish, ListingData, UserSpaces,
} from '../data/models';

async function checkListSettingsActivity(typeId, id) {
  let status,
    UserListingDataTable = [1, 3, 4, 7, 9];
  ...
  if (findActiveSettingsCount === 0) {
    ...
    } else if (typeId === 101) { // Fish
      const checkFishTable = await UserFish.count({
        where: {
          fishId: id,
        },
      });
      status = checkFishTable > 0 ? 'listingUsed' : status;
    } else {
