...
const sitemapRoutes = (app, routes) => {
  app.get('/sitemap.xml', async (req, res) => {
    ...
    const staticData = await Promise.all(routes.children.map(async (value, key) => {
    ...
    const listingDataURLs = await Promise.all(listingData.map(async (value) => {
      staticURLs.push({
        url: value,
        changefreq: 'daily',
        priority: 0.7,
    ...
    const listData = await Promise.all(listDetails.map(async (value) => {
      staticURLs.push({
        url: value,
        changefreq: 'daily',
        priority: 0.7,
  ...
  app.get('/update-currency-rate', async (req, res) => {
