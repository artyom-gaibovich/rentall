  ...
  render() {
    ...
    if (checkIn != null && checkOut != null) {
      ...
      if (dayDifference > 0) {
        ...
        for (let i = 0; i < dayDifference; i++) {
          const currentDate = moment(checkIn).add(i, 'day');
    ...
    if (dayDifference >= 7) {
      if (monthlyDiscount > 0 && dayDifference >= 28) {
        discount = (Number(priceForDaysTmp) * Number(monthlyDiscount)) / 100;
        discountType = `${monthlyDiscount}% monthly price discount`;
      } else if (weeklyDiscount > 0) {
        discount = (Number(priceForDaysTmp) * Number(weeklyDiscount)) / 100;
    ...
    const checkInStart = bookingData.listingData.checkInStart;
    const checkInEnd = bookingData.listingData.checkInEnd;
    ...
    totalWithoutFees = (priceForDaysTmp + cleaningPrice) - discount;

    const checkInDate = checkIn != null ? moment(moment(checkIn).format('YYYY-MM-DD')).format('ddd, Do MMM') : '';
    const checkOutDate = checkOut != null ? moment(moment(checkOut).format('YYYY-MM-DD')).format('ddd, Do MMM') : '';
    ...
    return (
      <Grid>
        <Row>
          ...
          { false && <Col lg={5} md={5} className={s.positionSticky}>
            <div className={cx(s.summaryCard, s.colCenter)}>
              ...
              <Panel className={s.panelHeader}>
                ...
                <div>
                  ...
                  <span className={s.policyColor}><FormattedMessage {...messages.noflexible} /></span>
                </div>
              </Panel>
            </div>
          </Col> }
