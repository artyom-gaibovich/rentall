...
const amenitiesUpload = (app) => {
  ...
  app.post('/uploadAmenities', (req, res, next) => {
    if (!req.user) {
      res.send(403);
    } else {
      next();
    ...
    await new Promise((resolve, reject) => {
      sharp(file.path)
        ...
        .toFile(`${amenitiesUploadDir}small_${file.filename}`, (err, file) => {
          if (file) {
            resolve(file);
          } else {
            reject(err);
    ...
    await new Promise((resolve, reject) => {
      sharp(file.path)
        ...
        .resize(255, 255)
        // .crop(sharp.strategy.entropy)
        .toFile(`${amenitiesUploadDir}medium_${file.filename}`, (err, file) => {
          if (file) {
            resolve(file);
          } else {
            reject(error);
  ...
  app.post('/deleteAmenities', (req, res, next) => {
    if (!req.user) {
      res.send(403);
    } else {
      next();
    ...
    const filePath = amenitiesUploadDir;
