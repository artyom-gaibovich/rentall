...
export function referralURL(data) {
  ...
  const splittedLocation = location.split('/')[1];
  queryParams = data && data.search ? data.search : '';
  const index = referralPages.findIndex(x => x === `/${splittedLocation}`);
  const locationValue = location.startsWith(referralPages);
  isReferralPage = index > -1;
  if (isReferralPage) {
...
export function getRedirectURL(listId, urlParameters) {
  ...
  if (urlParameters && urlParameters.listTitle && urlParameters.startDate && urlParameters.endDate) {
    let startDate = moment(urlParameters.startDate).format('YYYY-MM-DD'),
      endDate = moment(urlParameters.endDate).format('YYYY-MM-DD');
    const refer = `/rooms/${formatURL(urlParameters.listTitle)}-${listId}?&startdate=${startDate}&enddate=${endDate}&guests=${urlParameters.guests}`;
