  ...
  render() {
    ...
    return (
      <div className={cx(s.progressContainer, 'hidden-xs')}>
        ...
        <div
          className={cx(s.textTrunck, s.progressSection,
            s.progressStyle, { [s.active]: pathname === 'fish' })}
          title={formatMessage(messages.fish)}
        >
          <a className={cx(s.linkReset, s.progressStep)} onClick={() => nextPage('fish')} href="javascript:void(0);">
            <FormattedMessage {...messages.fish} />
          </a>
        </div>
