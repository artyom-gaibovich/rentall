  ...
  async resolve({ request, response }, {
    ...
    if (request.user || request.user.admin) {
      ...
      if (isListingAvailable != null) {
          // Currency
        const getCurrenice = await Currencies.findOne({
          where: {
            isBaseCurrency: true,
          },
        });

        const currencyValue = currency || getCurrenice.symbol;
        ...
        if (blockedDates) {
          ...
          if (blockedDatesData.length > 0) {
            ...
            blockedDates.map(async (item, key) => {
              ...
              const blockedItem = blockedDatesItems.map(Number).indexOf(+day);
              if (blockedItem > -1) {
