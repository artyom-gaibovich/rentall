...
const locationUpload = (app) => {
  ...
  app.post('/removeLocationFile', (req, res, next) => {
    if (!req.user) {
      res.send(403);
    } else {
      next();
    ...
    const filePath = locationuploadDir;
  ...
  app.post('/uploadLocation', (req, res, next) => {
    if (!req.user) {
      res.send(403);
    } else {
      next();
    ...
    const smallImage = await new Promise((resolve, reject) => {
      sharp(file.path)
        ...
        .toFile(`${locationuploadDir}small_${file.filename}`, (err) => {
          if (file) {
            resolve(file);
          } else {
            reject(error);
    ...
    if (file.mimetype === 'image/jpeg') {
      ...
      const mediumImage = await new Promise((resolve, reject) => {
        sharp(file.path)
          ...
          .jpeg({ quality: 50 })
          // .crop(sharp.strategy.entropy)
          .toFile(`${locationuploadDir}medium_${file.filename}`, (err) => {
            if (file) {
              resolve(file);
            } else {
              reject(error);
