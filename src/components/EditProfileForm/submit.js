...
async function submit(values, dispatch) {
  ...
  const monthValidation = parseInt(values.month);
  ...
  const firstNameValue = values.firstName ? values.firstName.trim() : values.firstName;
  const lastNameValue = values.lastName ? values.lastName.trim() : values.lastName;
  const phoneNumber = values.phoneNumber ? values.phoneNumber.trim() : values.phoneNumber;
  const location = values.location ? values.location.trim() : values.location;
  const infoValue = values.info ? values.info.trim() : values.info;
  const loggedinEmailValue = values.loggedinEmail ? values.loggedinEmail.trim() : values.loggedinEmail;
  const countryCode = values.phoneDialCode ? values.phoneDialCode : values.dialCode;
  const countryName = values.phoneCountryCode ? values.phoneCountryCode : null;
