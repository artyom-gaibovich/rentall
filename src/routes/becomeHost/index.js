...
export default {
  ...
  async action({ params, store, query }) {
    ...
    if (listId != undefined && !isNaN(listId)) {
      let response;
      // checkListing
      await store.dispatch(checkListing(listId, 'listing')).then((res) => {
    ...
    } else if (initialValuesLoaded != true || (mode && mode == 'new')) {
    ...
    if (listId != undefined && !isNaN(listId)) {
      ...
      const step3Pages = [
        'guest-requirements', 'house-rules', 'fish', 'review-how-guests-book',
