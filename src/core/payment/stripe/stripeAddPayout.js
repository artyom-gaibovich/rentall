...
const stripeAddPayout = (app) => {
  app.post('/stripe-add-payout', async (req, res) => {
  ...
  app.post('/stripe-verify-payout', async (req, res) => {
    const userDetails = req.body.userDetails;
    const account = userDetails && userDetails.currentAccountId;
    let status = 200,
      errorMessage,
      redirect;
    ...
    try {
      ...
      const accountUserId = getAccountDetails && getAccountDetails.metadata && getAccountDetails.metadata.userId;
