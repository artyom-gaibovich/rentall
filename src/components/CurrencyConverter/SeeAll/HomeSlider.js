      ...
      <Grid fluid>
        <Row>
          <Col xs={12} sm={12} md={12} lg={12} className={cx('noPadding', 'homeSliderPaading', 'homeSwiper')}>
            ...
            {
              load && <Swiper  noSwiping={true} rtl={isRTL(locale)} className={cx('row homeSlickSlider swiper-no-swiping', s.noMargin, s.noSwipe)} >
                {
                  data && data.length > 0 && data.map((item, index) => {
                    if (item.listPhotos.length > 0) {
                      return (
                        <div className={cx('col-md-12 col-sm-12 col-xs-12 swiper-no-swiping')} key={index}>
